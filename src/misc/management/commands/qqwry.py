...
# 解析 qqwry.dat 库
# Copyright: https://github.com/gwind/ylinux/tree/master/tools/IP/QQWry
...
def get_qqwry():
    """获取qqwry"""
    dbpath = os.path.join(os.path.dirname(__file__), './qqwry.dat')
    qqwry = QQWry(dbpath)
    return qqwry
...
class Command(BaseCommand):
    ...
    # todo: 支持command ip参数
    def start(self, ip='8.8.8.8'):
        qqwry = get_qqwry()
        c, a = qqwry.query(ip)
        self.stdout.write('%15s %s%s' % (ip, c, a), ending='\n')
